...
def get_yoga_moves(cursor):
    ...
    Assumes columns: id, name, side, HowToDoIt.
    """
    query = "SELECT id, name, side, HowToDoIt FROM yogamoves"
...
def is_yoga_move_processed(cursor, yoga_move_id):
    """
    Checks if the given yoga_move_id already has any mapping entries in the database.
    Returns True if there is at least one mapping, False otherwise.
    """
    query = "SELECT COUNT(*) FROM yogamovemusclemapping WHERE yoga_move_id = %s"
    cursor.execute(query, (yoga_move_id,))
...
def main():
    ...
    # Load database credentials from keyDB.txt
    ...
    try:
        ...
        for move in yoga_moves:
            # Check if this yoga move has already been processed
            if is_yoga_move_processed(cursor, move['id']):
                print(f"Yoga move id {move['id']} already processed. Skipping.")
                continue
